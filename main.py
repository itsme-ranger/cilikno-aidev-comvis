...
class Circle3PointsGenMousePack:
    def __init__(self, orig_height=None, frame_ith=-1) -> None:
        self.stored_points = []
...
def draw_circle_3points(event, x, y, flags, a: Circle3PointsGenMousePack):
    ...
    if (event == cv2.EVENT_LBUTTONDOWN) & (len(a.stored_points) <= 1):
        print("Left button of mouse has been single clicked")
        ...
        print("Left button of mouse has been single clicked with len==2")
        a.stored_points.append([x, y])
...
obj = Circle3PointsGenMousePack(orig_height=vidcap.get(4))
...
while vidcap.isOpened():
    ...
    if ret:
        ...
        if VISUALIZE_IT:
            ...
            ch = cv2.waitKey(int(1000/vidcap.get(cv2.CAP_PROP_FPS)/_SPEED_PLAYBACK))
            if ch & 0xFF == ord('q'):
                break
